...
#             optimize_image_for_ocr(temp_path)
#             img = Image.open(temp_path).convert('L')# Speeds up OCR
...
# PythonAnywhere Tesseract Configuration
...
# --- AI SCANNING UTILITIES ---
def optimize_image_for_ocr(image_path):
    """Pre-processes image to improve OCR detection accuracy."""
    with Image.open(image_path) as img:
        if img.width > 2000 or img.height > 2000:
            img.thumbnail((2000, 2000))
        # Convert to Grayscale (L mode) helps Tesseract read text better
        img = img.convert('L')
...
def is_luhn_valid(card_number):
    ...
    if len(card_number) < 13: return False
    digits = [int(d) for d in card_number]
    for i in range(len(digits) - 2, -1, -2):
        digits[i] *= 2
...
def scan_content(text):
    ...
    for card in potential_cards:
        if is_luhn_valid(card):
            leaks.append("Credit Card")
            break

    # 2. PAN Card (Standard Indian Format)
    if re.search(r'[A-Z]{5}[0-9]{4}[A-Z]', text):
        leaks.append("PAN Card")
    ...
    # 4. Bank Details
    if re.search(r'(?i)(account|a/c|acc)[\s:#-]*(\d{9,18})', text):
        leaks.append("Bank Account")
    if re.search(r'[A-Z]{4}0[A-Z0-9]{6}', text):
        leaks.append("IFSC Code")

    # 5. General Secrets
    if re.search(r'(?i)(password|secret|apikey|token|passwd)', text):
        leaks.append("System Secret")
...
def upload_file():
    ...
    temp_path = os.path.join(UPLOAD_FOLDER, "proc_" + file.filename)
    file.save(temp_path)
    ...
    try:
        ...
        if found_leaks:
            ...
            new_log = AuditLog(
                ...
                status="Rejected", 
                details=f"Sensitive data found: {leak_details}"
            )
            db.session.add(new_log)
            db.session.commit()
            os.remove(temp_path)
            flash(f"🚨 AI BLOCK: {leak_details} detected in {file.filename}.")
            return redirect(url_for('index'))
    ...
    new_log = AuditLog(
        ...
        action="Upload", 
        status="Success", 
        details=f"File {file.filename} encrypted."
    )
    db.session.add(new_file)
    db.session.add(new_log)
    db.session.commit()
...
def download_file(file_id):
    ...
    if not os.path.exists(file_path):
        flash("Physical file not found in storage.")
        return redirect(url_for('index'))
    ...
    return send_file(
        io.BytesIO(cipher.decrypt(enc_data)), 
